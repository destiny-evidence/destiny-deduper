...
class Deduper:
    ...
    def __init__(
        ...
        # Optional logistic-derived attribute weights (dict: field -> weight)
        self.attribute_weights: dict | None = None
    ...
    def compare_one_to_one(
        ...
        **kwargs,
        ...
        # If attribute weights are available (from a trained logistic regression),
        # compute a weighted linear combination and pass through the logistic sigmoid
        # to produce a probability-like score. Otherwise fall back to simple mean.
        if self.attribute_weights:
            # Ensure consistent ordering; include only fields we computed scores for
            linear = float(self.intercept or 0.0)
            for field, score in scores.items():
                w = self.attribute_weights.get(field, 0.0)
                linear += w * float(score)
        ...
        # Fallback: simple mean of available scores
        if len(scores) == 0:
            return 0.0
