...
def test_compare_isbn_exact_match():
    ...
    assert deduper.compare_isbn(paper1, paper2) == 1.0
...
def test_compare_isbn_no_match():
    ...
    paper2 = Paper(isbn="8537809667")
    deduper = Deduper(reference=paper1, candidates=[paper2])
    assert deduper.compare_isbn(paper1, paper2) == 0.0
...
def test_compare_year_exact_match():
    ...
    deduper = Deduper(reference=paper1, candidates=[paper2])
    assert deduper.compare_year(paper1, paper2) == 1.0
...
def test_compare_year_no_match():
    ...
    deduper = Deduper(reference=paper1, candidates=[paper2])
    assert deduper.compare_year(paper1, paper2) == 0.0
...
def test_compare_pubmed_id_exact_match():
    ...
    deduper = Deduper(reference=paper1, candidates=[paper2])
    assert deduper.compare_pubmed_id(paper1, paper2) == 1.0
...
def test_compare_pubmed_id_no_match():
    ...
    deduper = Deduper(reference=paper1, candidates=[paper2])
    assert deduper.compare_pubmed_id(paper1, paper2) == 0.0
